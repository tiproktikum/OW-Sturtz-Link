...
- **Документация формата:** `Клиенты/Контур, Тула, OW8/Документация Sturtz/ProOpt_DSatz_V205_Staba Russisch.doc`
- **Статус:** это официальная детальная документация по формату принимаемого файла; использовать как основной источник формата.
- **Позиции кодов .sob (VER02.07):** `temp/Позиции_кодов_SOB_наша_версия.md` — наша рабочая привязка позиций (B на 32, O на 53 и т.д.) для совместимости с Pro OPT; относится к версии 02.07, а не к 02.05 из документации.
...
Требование к имени файла выгрузки: **не более 8 символов** (до расширения). Формат для всех выгружаемых файлов:

- **G** + наименование группы оптимизации (транслит, обрезанное до **7 символов**) + **.SOB**

Пример: группа «Двери кухня» → `GDveri k.SOB`. Расширение — всегда **.SOB** (верхний регистр). При реализации любых видов выгрузки (.sob, .soe и т.д.) соблюдать это ограничение по длине имени.
...
7. **Имя файла выгрузки:** максимум 8 символов до расширения; формат **G** + до 7 символов наименования группы (транслит) + **.SOB**. Для всех видов выгружаемых файлов — то же ограничение.
